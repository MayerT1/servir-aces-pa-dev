...
class TrainingDataGenerator:
    ...
    def __init__(self, config, split_dir: str = "training", **kwargs):
        ...
        **kwargs: Additional keyword arguments to pass to the class.
        ...
        self.sample_locations = ee.FeatureCollection(kwargs.get("sampled_locations",
                                                                        "users/tjm0042/PHD/GEDI_Height/training_GEDI"))


        self.sample_locations = self.sample_locations.randomColumn("random", self.seed)

        default_label = ee.Image('users/tjm0042/PHD/GEDI_Height/stackedResampled').select("class").unmask(0, false)
    ...
    def load_data(self) -> None:
        ...
        if self.config.USE_S1:
            ...
            self.config.FEATURES.extend(["B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8", "B8A", "B9", 
                                         "B11","B12","ndvi","mndwi", "ndbi", "evi", "bsi", "dem", "slope", 
                                         "rh100", "NDWI_HLS", "MNDWI_HLS", "SAVI_HLS", "NDMI_HLS","NDBI_HLS", 
                                         "NDWI_LS", "MNDWI_LS", "SAVI_LS", "NDMI_LS","NDBI_LS", "brightness", 
                                         "greenness", "wetness", "fourth", "fifth", "sixth","VV_A", "VH_A", "ratio_A",
                                          "ndratio_A", "VV_D", "VH_D", "ratio_D", "ndratio_D", "rh100_1", "class", ])
...
if __name__ == "__main__":
    ...
    # sample locations
    sample_locations = "users/tjm0042/PHD/GEDI_Height/training_GEDI"
    ...
    if parser.parse_args().label_data:
        ...
        label = ee.Image("users/tjm0042/PHD/GEDI_Height/stackedResampled").select("class").rename("class").unmask(0, False)
